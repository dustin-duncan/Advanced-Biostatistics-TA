...
* Quantify the posterior distribuiton using mean, median, quantiles, and HPDI
...
```{r}
# Have samples that are sampled from the grid approximation
# Going to use the samples because that is the truest proportion of water on earth
...
```{r}
# In red is the distribution two graphs above 
  # includes uncertainty from our estimate and from the binomial draw 
  # you would want to include both of these variances in your decision making in a future problem
# In blue is the distribution just using the one parameter (median from above)
...
```{r}
# Spend some time plotting histogram of it, and calculating the metrics on it. Then ask yourself what those metrics are showing you 

load("distributions_data.RDS")
data1
view(data1)
...
stats <- dist_quant(data1$value)

stats1 <- dist_quant(data2$value)
```

...
```
# Bayesian Statistical Modeling Winter 2024 blah
# Lab Exercise, Week 4
...
Even though mu is not a parameter anymore, because it is a function of parameters a and B, it still has a posterior distribution, because the parameters that make it up have posterior distributions. 

Working with samples from the posterior allows you to account for the posterior uncertainty of any quantity by 
...
Here remember that our model doesn't make a dummy variable for ourselves so we have to specify two different scenarios based off of the same model where in one, G is at its mean, and in the other F is at its mean. 

...
samples1.1 <- sample_n(post_approx, weight = posterior, size = 1e4, replace = TRUE) %>% 
  select(p)
...
```{r , eval=TRUE}
ggplot(data=samples1.1 , aes(x=p))+ geom_histogram(bins=30) +
  labs(title = "Samples 1.1")

ggplot(data=samples1, aes(x = p)) + geom_histogram(bins = 30) + 
  labs(title = "Samples 1")
...
ggplot(data=samples2 , aes(x=p))+ geom_histogram(bins=30)
```
...
sum(samples1.1$p < 0.2) / 1e4

sum(samples2$p < 0.2) / 1e4
```
...
PI <- PI(samples1$p, prob = 0.66)

PI2 <- tibble(plow = PI[1]) %>% 
  mutate(phigh = PI[2],
         diff = PI[2] - PI[1])
...
plot(post_approx2$posterior ~ post_approx2$p, type = "l")
```
...
```{r ,eval=TRUE}
# Answer: ~0.1423
new <- rbinom(1e4, size = 15, prob = samples3$p)
...
```{r ,eval=TRUE}
# Answer: ~0.1746
new1 <- rbinom(1e4, size = 9, prob = samples3$p)
mean(new1 == 6)
...
```{r}
PI <- PI(samples4$p, prob = 0.99)
 PI_interval <- tibble(lower = PI[1]) %>% 
   mutate(higher = PI[2],
          difference = PI[2] - PI[1])
```

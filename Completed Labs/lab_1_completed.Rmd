...
# A named list of the three variables (see https://www.geeksforgeeks.org/named-list-in-r-programming/ for info on named lists)
...
The sign of a real number is 1, 0, -1, which corresponds to positive, zero, or negative
...
The numbers change each time we run it because the function is based off of random draws from a binomial distribution. Each time the draws change so the value of the mean changes.

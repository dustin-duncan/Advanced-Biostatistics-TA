...
#### Grid Approximation: Approximating the posterior distribution. 
  - At any value of parameter p', we want to compute its posterior probability. 
    - Do this by multiplying the prior probability of p' by the likelihood at p'
  - Repeat for each value in the grid --> get an exact picture of the posterior 
  distribution

What is p? p is the percentage of earth's surface that is water. 

Recipe for grid approximation: 

1) Define the grid. How many points to use in estimating the posterior, and then make a list of parameter values on the grid.

2) Compute the likelihood at each parameter value
 
3) Compute the value of the prior at each parameter value on the grid 

4) Compute the unstandardized (raw) posterior at each parameter value, by multiplying the prior by the likelihood. 

5) Standardize the posterior --> divide each value by the sum of all values 

```{r}
# define your grid (probability of water) (if there is a 60% probability of landing on water, what proportion of Earth is covered in water?)
p <- seq(from = 0, to = 1, length.out = 1000)

p2 <- seq(from = 0, to = 1, by = 0.001)

# Compute likelihood at each parameter value (how likely is it that we draw 6 waters from 9 tosses at each given value of p (proportion of water on earth))
likelihood <- dbinom(6, size = 9, prob = p)
likelihood2 <- dbinom(6, size = 9, prob = p2)

# Define your prior probability (saying that its uniform and at 1) (this informs our posterior distribution)
prior <- rep(1, 1000)


# Compute product of likelihood and prior (shape of prior likelihoods)

unstd.posterior <- likelihood * prior

# Standardize the posterior, so it sums to 1  (The likelihood of some value of p having generated our observed 6 waters in 9 tosses) (The probability of your value of p being the statistical parameter underlying your observed 6 waters in 9 tosses)
# Posterior probabilities state the relative number of ways each conjectured cause of the data (in this case value of p) could have produced the data. These indicate the relative plausibilities of the different conjecture

posterior <- unstd.posterior /sum(unstd.posterior)

# Create a plot of your posterior distribution 
...
ggplot(data=post_approx, aes(x = p, y = posterior)) + 
  geom_point() + 
  geom_segment(aes(x = p, xend = p, y = 0, yend = posterior))
```
...
samples <- tibble(p = sample(p, prob = posterior, size = nsamp, replace = TRUE)) 
# For this code, sample is pulling random values of p (percentages of water on earth) from a vector. The vector is p_grid, and the probability of each value comes from the posterior distribution, which we just computed.
...
sum(samples$p < 0.2) /1e4

# If you didnt divide by 1e4 then you would just be adding up p values, but you divide by the total number of p-values (10000) to get the proportion of those in the vector (posterior distribution)
```
...
```{r}
# Answer: ~0.8856
sum(samples$p > 0.2 & samples$p< 0.8) /1e4
```
...
post_approx2 <- tibble(p = seq(from = 0, to = 1, length.out = 1e4)) %>% 
  mutate(likelihood = dbinom(8, size = 15, prob = p_grid),
         prior = rep(1, 1e4),
         raw.posterior = likelihood * prior,
         posterior = raw.posterior/sum(raw.posterior))

ggplot(post_approx2, aes(x = p, y = posterior)) + 
  geom_point()
```
...
samples <- tibble(p = sample(post_approx2$p, prob = post_approx2$posterior, size = 1e4, replace = TRUE))
...
post.sims.long <- post.sims %>% 
  pivot_longer(col = c(6:7),
               names_to = "type",
               values_to = "Ws")
...
```{r ,eval=FALSE}
# Answer: ~0.1746
post.sims.new <- post.sims %>% 
  mutate(Wsampled2 = rbinom(1e4, size = 9, prob = samples$p))

mean(post.sims.new$Wsampled2 == 6)

Wsampled2 <- rbinom(1e4, size = 9, prob = samples$p)
...
# Take ssamples and compute the 90% HPDI

# Conduct posterior predictive simulation of model and compute likelihood of 8 waters in 15 tosses 

# Use either gather or pivot_longer to get both binomial values and sample values in the same dataframe so you can plot them against each other (checking to see if samples make sense)

# Conduct posterior predictive check of your model

# Compute the likelihood of finding 8 waters in 15 tosses 
...
int.width <- function(N) {
  ...
  # You can write this as an object or as a tibble. 
  
  ## 1) *< 'object name' <- 'code to calculate percent interval' >* 
  # **this will be stored in the values pane in your environment
  
  ## 2) *< 'tibble name' <- 'create tibble with one bound of your PI' %>%  'mutate to add other bound of PI in a second column and the difference between them in a third column' 
<<<<<<< HEAD
  # **this will be stored in the data pane in your environment. Look at the data types - is your difference column numeric? If not, how do you change it to numeric data type? 
=======
  # **this will be stored in the data pane in your environment
>>>>>>> 4792e918eed300f35ce748bdedcdcaca690b9028

  # Calculate the width of that interval
  # < write code here >
  
  ## 1) calc the interval by using index numbers from your PI object directly
  
<<<<<<< HEAD
  ## 2) calc the interval through your tibble that you just created (specify the tibble (vector) you're pulling from and the column) 
=======
  ## 2) calc the interval through your tibble that you just created (specify the tibble (vector) you're pulling from and the column)
>>>>>>> 4792e918eed300f35ce748bdedcdcaca690b9028

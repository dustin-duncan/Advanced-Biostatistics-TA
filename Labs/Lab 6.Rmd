...
for ( i in 1:num_weeks ) {
  ...
  ## now make sure he loops around the archipelago (cant go bigger than 10)
  ...
  # generate a random number between zero and one --> the king moves if this 
  # Random number is less than the ratio of the proposal island's population to
  # the current islands population (proposal/current)
  current <- ifelse( runif(1) < prob_move , proposal , current)
...
Make up your own "population size surface". You can make it longer or shorter. Create a set of plots as we did for the other surfaces and guage how well the algorithm was able to match your function.

Creating my own population surface 
```{r}
n <- seq(from = 10, to = 500, by = 10)

popsize <- tibble(location = rep(1:50), population = sample(n, 50, replace = T))
num_islands = nrow(popsize)
```
...
```{r}
ggplot(popsize, aes(x=location,y=population)) + geom_line()
```

```{r, eval=FALSE}
norm=sum(popsize$population) 

popsize <- mutate(popsize,frequency = population/norm)

sum(popsize$frequency) # good it adds up to one 
```
...
Start off at island 30 and set the value of current to the current position:
```{r, eval=FALSE}
positions <- tibble(week=1,position=30)
...
With this random seed, we move right, i.e. from 30 to 31
Now get the position of the proposal move
```{r, eval=FALSE}
 (proposal <- current + flip)
...
But 31 might not be a real position, because we might have wrapped around the top of our circle of islands, so we check this and figure out that we are going to island 31:
```{r, eval=FALSE}
 if ( proposal < 1 ) proposal <- num_islands
    if ( proposal > num_islands ) proposal <- 1
...
Now we calculate the probability of moving, which is one if the proposal island has a larger size, and is the ratio of the population sizes on the islands we are comparing otherwise. First we calculate the ratio of populations sizes:
```{r, eval=FALSE}
 (prob_move <- popsize[popsize$location==proposal,]$population / popsize[popsize$location==current,]$population)
```
Island 31 is 0.125 as big as island 30, so we have a 12.5% chance of moving to that island.

We move if a random draw (uniform(0,1)) is less than the value of `prob_move`
```{r, eval=FALSE}
(current <- ifelse( runif(1) < prob_move , proposal , current ))
```
Our random draw was bigger than 0.125, so we stay on island 30.
...
ggplot(data=positions, aes(x=location))+
  ...
  geom_line(data=tibble(location=seq(1:50),count=seq(1:50)) %>%
              mutate(count=count*num_weeks/norm),inherit.aes = F, aes(x=location,y=count))
```

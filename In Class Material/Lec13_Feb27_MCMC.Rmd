...
Say you have some wack data that was given to you: 
```{r}
wack <- tibble(sample_n = c(1, 2, 3, 4), treat = c("C", "T", "C", "T"), N = c(20, 23, 18, 25), n_alive = c(11, 17, 10, 20), treat_index = c('1', '2', '1', '2'))
head(wack)
```

We want to add some indicator variables to this table so that we can add in treatment effect into the model: 
```{r}
wack_new <- wack %>% 
  mutate(I_c = if_else(
    treat_index == '1', 1, 0),
...
Our model formula for this would be:

$$
\mathrm{(A)live_i \sim Binomial(N_{i} , P_{i})} \\ 
\mathrm{Logit(P_{i}) = \mu_{i}} \\
\text{Choose your own method:} \\
M_1 ->~ \mu_{i} = a + b*I_{t} \\ or \\
\mu_{i} = a + b*I_{c} \\ 
M_2 ->~ \mu_{i} = a_{1}*I_{c} + a_{2}*I_{t} \\ 
M_3 ->~ \mu_{i} = a + b[T_{i}] \\ 
M_4 ->~ \mu_{i} = a + \triangle{a}*I_{c} - \triangle{a}*I_{t} \\ 
M_5 ->~ \mu_i = a + b*T_{i}
$$

M1 - in M_1, you can choose a value of A and B, that will give you the same output as m2
  - a now represents the control and b is the difference when you have the 
  treatment 
  - now because a is the reference value when I_t = zero, so even in the second version its the same just your baseline is different


M2 - Splitting our intercept in M_2 will automatically incoroporate our Indicator variables values when treatment is either 'treatment' or 'control'

M3 - Now here a would be the average survivorship and B is what you get outside of the average 
  - As long as the difference between the B's is the same, the specific
  values of a and b are not super important (unidentifiable)
  - There will be covariance between the distribution of a and both values 
  of b
  
M4 - a is the midpoint between our two treatments, and the delta is the amount that it goes up or down when you have one treatment versus the other.
  - Your prior for delta(a) is that it would be centered around zero whereas
  your prior for a is that it would be broad 
  
M5 - for treat 1 you get a + b, but for treat 2 you get a + 2*b
  - But now the parameters are not readily interpretable 
  - Could rescale it so the values of the index were -1 and +1, then it would
  become interpretable and function the same as M4 
